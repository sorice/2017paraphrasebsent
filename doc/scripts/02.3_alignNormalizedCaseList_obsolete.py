    ...
    if len(sys.argv) == 5:
        ...
        for line in doc.split('\n'):
            if line[line.find(' ')+1:] == 'True':
                alignedDocList[line[:line.find(' ')]] = bool('True')
            else:
                if len(line) > 1:
                    alignedDocList[line[:line.find(' ')]] = bool('')
        txt.close()
        ...
        for i,line in enumerate(lines):
            ...
            if i/cases%0.1 == 0.0 or i == cases:
                file = open(outdir+'alignedDocList')
                aligned_docs = file.readlines()

                for tex in aligned_docs:
                    try:
                        value = tex.split()[0]
                        if len(value) > 1:
                            dicc[value] = True
